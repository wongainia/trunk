...
public class ClipZoomImageView extends ImageView implements
	...
	private final int SCALE_DELAY = 16;// 缩放动画的时间
	...
	private class AutoScaleRunnable implements Runnable {
		...
		public void run() {
			...
			if (((tmpScale > 1f) && (currentScale < mTargetScale))
				...
				ClipZoomImageView.this.postDelayed(this, SCALE_DELAY);
			} else
			// 设置为目标的缩放比例
			{
	...
	public void onGlobalLayout() {
		if (once && isScreenOriatationPortrait()) {
			Drawable d = getDrawable();
			if (d == null)
				return;
			// 垂直方向的边距
	...
	private static boolean isScreenOriatationPortrait() {
		return ACGApplication.getInstance().getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT;
	}
